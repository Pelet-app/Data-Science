...
# Direktori tempat data_loader.py berada
...
def load_jobs() -> pd.DataFrame:
    df = pd.read_csv("DATA_DIR/cleaned_job_5.csv")
    ...
    skill_map_df = pd.read_csv("DATA_DIR/skill_mapping_dictionary.csv")
...
def load_resumes() -> pd.DataFrame:
    df = pd.read_csv("DATA_DIR/cleaned_training_data.csv")
...
def load_skkni() -> pd.DataFrame:
    df = pd.read_csv("DATA_DIR/skkni_reference_clean.csv")
    return df


def load_skill_map() -> pd.DataFrame:
    df = pd.read_csv("DATA_DIR/skill_mapping_dictionary.csv")
    return df
